...
#define PROTOCOL_SERVER_MESSAGE_SIZE   (2/*magic*/ + 2/*length*/ + 1/*message_type*/ +  8/*timestamp*/+  2/*counter*/ + PROTOCOL_PAYLOAD_SIZE)
...
typedef union {
    uint8_t buffer[PROTOCOL_SERVER_MESSAGE_SIZE];
    struct {
        uint16_t magic;
        uint16_t length;
        uint8_t message_type;
